...
lang_choice = st.sidebar.selectbox(
    "🌍 Language",
    ["English", "French", "German", "Spanish", "Telugu", "Hindi"]
)
lang_map = {
    "English": "en",
    "French": "fr",
    ...
    if st.session_state.language != "en":
        try:
            translated = GoogleTranslator(
                source=st.session_state.language, target="en"
            ).translate(user_input)
        except Exception:
            translated = user_input  # fallback if translation fails
    else:
    ...
    if st.session_state.language != "en":
        try:
            response = GoogleTranslator(
                source="en", target=st.session_state.language
            ).translate(response)
        except Exception:
            pass  # if translation fails, keep English
